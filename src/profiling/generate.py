...
async def send_request(client: httpx.AsyncClient, url: str, model_name: str, messages: List[str], mid: str) -> Optional[Dict[str, Any]]:
    """
...
async def main():
    ...
    output_path = output_dir / f"{args.model.lower().split("/")[1]}-port-{args.port}-{dataset_dir.name}-generated.jsonl"
        ...
        with open(output_path, "w", encoding="utf-8") as f:
            ...
            for row in dataset:
                ...
                if len(messages) >= 2:
                    ...
                    task = bounded_request(sem, client, vllm_url, args.model, openai_messages, mid)
                    tasks.append(task)
            ...
            for future in asyncio.as_completed(tasks):
                ...
                if result_data is not None:
                    ...
                    print(f"[{completed_count}/{total_rows}] [Done] MID: {mid} | Tokens: {tokens}")
