...
def main():
    ...
    if not all_concurrency:
        print("No valid data points extracted. Ensure JSON files contain 'max_concurrency' and nested 'itls' fields.")
        return
    ...
    plt.scatter(X, Y * 1000, alpha=0.2, label='Request Mean ITL Samples', color='blue', s=15)
    
    # Calculate global means per N for trend clarity
    unique_n = np.unique(X)
    global_means_y = [np.mean(Y[X == n]) * 1000 for n in unique_n]
    plt.scatter(unique_n, global_means_y, color='black', marker='x', label='Global Mean per N', s=60, zorder=3)
    
    # Plot regression line (converted to ms)
    line_x = np.array([min(X), max(X)])
    line_y = (d * line_x + c) * 1000
    plt.plot(line_x, line_y, color='red', linewidth=2, label=f'Fit: y = {d*1000:.4f}N + {c*1000:.2f}ms', zorder=4)
